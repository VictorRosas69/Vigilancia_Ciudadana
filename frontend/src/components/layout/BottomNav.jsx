  ...
  return (
    ...
    >
      <div className="mx-auto max-w-lg px-4 pb-2 pt-1">
        <div className="bg-white/95 dark:bg-gray-900/97 backdrop-blur-xl border border-gray-200/50 dark:border-gray-700/60 rounded-2xl flex items-center justify-around px-2 py-1"
          ...
          {NAV_ITEMS.map((item) => {
            ...
            return (
              <NavLink key={item.to} to={item.to} end={item.end} className="flex-1" onClick={() => haptic.light()}>
                {({ isActive }) => (
                  <div className="flex flex-col items-center gap-0.5 py-2 relative">
                    <div className={`p-1.5 rounded-xl transition-all duration-200 relative ${isActive ? 'bg-blue-50' : ''}`}>
                      <motion.div
                        key={isActive ? 'on' : 'off'}
                        initial={isActive ? { scale: 0.6, y: 5 } : false}
                        animate={{ scale: 1, y: 0 }}
